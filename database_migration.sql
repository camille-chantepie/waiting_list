-- ============================================
-- MIGRATION AKADEMOS - Ajout des nouvelles tables
-- ============================================
-- Ce script ajoute uniquement les nouvelles tables sans toucher aux existantes

-- ============================================
-- TABLE AKADEMOS (newsletter) - Vérifier et créer
-- ============================================

-- Créer la séquence si elle n'existe pas
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'akademos_id_seq') THEN
    CREATE SEQUENCE public.akademos_id_seq;
  END IF;
END $$;

-- Créer la table si elle n'existe pas
CREATE TABLE IF NOT EXISTS public.akademos (
...
ALTER SEQUENCE public.akademos_id_seq OWNED BY public.akademos.id;

-- ============================================
-- MODIFICATION DE LA TABLE TEACHERS
-- ============================================

-- Ajouter le champ code_invitation si il n'existe pas déjà
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.teachers ADD COLUMN code_invitation text UNIQUE;
  END IF;
END $$;

-- Index pour améliorer les recherches par code
CREATE INDEX IF NOT EXISTS idx_teachers_code_invitation ON public.teachers(code_invitation);

-- ============================================
-- NOUVELLE TABLE: teacher_student_relations
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_relations_teacher ON public.teacher_student_relations(teacher_id);
CREATE INDEX IF NOT EXISTS idx_relations_student ON public.teacher_student_relations(student_id);
...
CREATE INDEX IF NOT EXISTS idx_messages_relation ON public.messages(relation_id);
...
-- Index pour améliorer les performances
CREATE INDEX IF NOT EXISTS idx_time_slots_relation ON public.time_slots(relation_id);
...
-- Index pour améliorer les performances
CREATE INDEX IF NOT EXISTS idx_documents_relation ON public.documents(relation_id);
CREATE INDEX IF NOT EXISTS idx_documents_type ON public.documents(document_type);
CREATE INDEX IF NOT EXISTS idx_documents_created_at ON public.documents(created_at DESC);

-- ============================================
-- NOUVELLE TABLE: grades
-- ============================================
...
-- Index pour améliorer les performances
CREATE INDEX IF NOT EXISTS idx_grades_relation ON public.grades(relation_id);
...
DROP TRIGGER IF EXISTS trigger_set_teacher_invitation_code ON public.teachers;
...
-- Supprimer les anciens triggers si ils existent
DROP TRIGGER IF EXISTS update_students_updated_at ON public.students;
DROP TRIGGER IF EXISTS update_teachers_updated_at ON public.teachers;
DROP TRIGGER IF EXISTS update_relations_updated_at ON public.teacher_student_relations;
...
DROP POLICY IF EXISTS "Anyone can view teacher codes for joining" ON public.teachers;
DROP POLICY IF EXISTS "Users can view their relations" ON public.teacher_student_relations;
DROP POLICY IF EXISTS "Students can create relations" ON public.teacher_student_relations;
DROP POLICY IF EXISTS "Teachers and students can update their relations" ON public.teacher_student_relations;
DROP POLICY IF EXISTS "Users can view messages in their relations" ON public.messages;
DROP POLICY IF EXISTS "Users can send messages in their relations" ON public.messages;
DROP POLICY IF EXISTS "Users can update their own messages" ON public.messages;
DROP POLICY IF EXISTS "Users can view time slots in their relations" ON public.time_slots;
DROP POLICY IF EXISTS "Users can create time slots in their relations" ON public.time_slots;
DROP POLICY IF EXISTS "Users can update time slots in their relations" ON public.time_slots;
DROP POLICY IF EXISTS "Users can view documents in their relations" ON public.documents;
DROP POLICY IF EXISTS "Users can upload documents in their relations" ON public.documents;
...
DROP POLICY IF EXISTS "Teachers can create grades for their students" ON public.grades;
DROP POLICY IF EXISTS "Teachers can update their students' grades" ON public.grades;
DROP POLICY IF EXISTS "Teachers can delete their students' grades" ON public.grades;
...
CREATE POLICY "Teachers can create grades for their students" ON public.grades
  FOR INSERT WITH CHECK (
    ...
    AND EXISTS (
      SELECT 1 FROM public.teacher_student_relations
      WHERE teacher_id = auth.uid() AND student_id = grades.student_id
    )
  );
...
DROP VIEW IF EXISTS public.relations_with_details;

CREATE VIEW public.relations_with_details AS
SELECT 
  r.id as relation_id,
  r.status,
  ...
  t.code_invitation as teacher_code,
  s.id as student_id,
...
DROP VIEW IF EXISTS public.student_grade_statistics;

CREATE VIEW public.student_grade_statistics AS
SELECT 
  student_id,
  subject,
...
-- ============================================
-- GÉNÉRER DES CODES POUR LES PROFS EXISTANTS
-- ============================================

-- Mettre à jour les professeurs existants qui n'ont pas de code d'invitation
UPDATE public.teachers
SET code_invitation = generate_invitation_code()
